...
def _replace_video_token_placeholders(
    ...
    Args:
        ...
        target: Token-ID sequence to search for. New-style processors expand
            only ``[video_pad_id]``; older ones expand the full
    ...
    while i < n:
        if prompt_ids[i : i + target_length] == target:
...
class Qwen3VLMultiModalProcessor(BaseMultiModalProcessor[Qwen3VLProcessingInfo]):
    ...
    def _expands_only_video_token(hf_processor: ProcessorMixin) -> bool:
        """Check whether the processor expands only the bare video token."""
